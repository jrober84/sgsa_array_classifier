...
include __DIR__."/readRawData.php";
ini_set("memory_limit","8000M");

/*
$profile_file = '/Users/jrobertson/Desktop/Classifier_Data/SGSA_V2_Data.txt';
$probe_config = '/Users/jrobertson/Desktop/Classifier_Data/probe.config';

$dir = '/Users/jrobertson/Desktop/SGSA v2.0 data/CFIABaselineJR/';
$files = glob($dir."*");
$obj = new buildProfile($profile_file,$probe_config);
$sero = new serovarLookUp('/Users/jrobertson/Desktop/serovars.txt');
...
foreach($files as $file)
{
    ...
    $result = $obj->classify($raw->data, $obj->profiles,$obj->probe_obj );
    $serovar = $sero->getSerovar($result['O'],$result['H1'],$result['H2']);
    $out_string = '';
    foreach($serovar as $name => $formula)
    {
        $out_string.= $name.' ('.$formula.')|';
    }
...
class buildProfile
{
    ...
    function classify($query_profile,$target_profiles,$probe_obj)
    {
        ...
        //var_dump($this->CategoryVote($this->getKNN($query_profile,$target_profiles['Records'],15)));
        //die;
       ...
       // var_dump($h1Match);
        ...
        if(($h1Match['antigen'] == 'l,v' || $h1Match['antigen'] == 'l,w') 
                && $h2Match['antigen'] != 'e,n,z15' && substr($h2Match['antigen'],0,1) != '1')
        {
        ...
        if($oMatch['antigen'] == 'D')
        {
            ...
            if(array_key_exists('210_Pullorum-1',$ranked_probes['Serovar'] ) && ($h1Match['antigen'] == 'g,m' || $h1Match['antigen'] == 'g,q' || $h1Match['antigen'] == 'g,p'))
            {
                //$h1Match['antigen'] = '-';
            }
            ...
            if(!array_key_exists('213_Enteritidis-1', $ranked_probes['Serovar']) && $h1Match['antigen'] == 'g,m')
            {
                if($h1Match['antigen'] == 'g,m' && $h2Match['antigen'] == '-'
                        && array_key_exists('gmq-gq-5', $ranked_probes['H1'])
                        && array_key_exists('137_RHS-Gallinarum-2', $ranked_probes['Serovar'])
        ...
        if(array_key_exists('Biotin-Marke_2,5?M', $query_profile))
        {
            $query_profile['Biotin-Marke_2,5µM'] = $query_profile['Biotin-Marke_2,5?M'];
            unset($query_profile['Biotin-Marke_2,5?M']);
            //var_dump($query_profile);
        }
        if(($query_profile['120_invA'] < $thresholds['120_invA']) || ($query_profile['Biotin-Marke_2,5µM'] < $thresholds['Biotin-Marke_2,5µM'] ))
        {
